...
import type { UpdateAdData, Ad } from '../types';

interface EditAdModalProps {
  isOpen: boolean;
  onClose: () => void;
  ad: Ad;
  onUpdateAd: (data: UpdateAdData) => Promise<void>;
}

export function EditAdModal({ isOpen, onClose, ad, onUpdateAd }: EditAdModalProps) {
  ...
  const [targetArticleSlug, setTargetArticleSlug] = useState(ad.target_article_slug || '');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTargetArticleSlug(ad.target_article_slug || '');
      setStatus(ad.status);
      setError(null);
    }
  }, [isOpen, ad]);
      ...
      <div className="fixed inset-0 z-[101] flex items-center justify-center pointer-events-none p-4">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6 p-6 pb-0">
            <h2 className="text-2xl font-bold text-black">Edit Ad</h2>
            <button
              onClick={handleClose}
              disabled={isUpdating}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-6">
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                type="submit"
                disabled={isUpdating || !imageUrl.trim() || !linkUrl.trim() || !headline.trim()}
