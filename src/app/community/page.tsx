  ...
  const loadPins = useCallback(async () => {
    ...
    try {
      ...
      loadedPins.forEach((pin) => {
        ...
        // Add click event handler to marker element to prevent map click when clicking on pin
        ...
        if (markerElement) {
          ...
          // Prevent map click on hover (mouseenter/mouseleave don't need stopPropagation, but good to be explicit)
          markerElement.addEventListener('mouseenter', () => {
            // Marker hover - no action needed, but prevents accidental map interactions
          });
        }

        // Add click event handler to popup content to prevent map click when clicking inside popup
        popup.on('open', () => {
          const popupElement = popup.getElement();
          if (popupElement) {
            // Stop propagation for clicks inside popup
            popupElement.addEventListener('click', (e) => {
              e.stopPropagation();
            });

            // Stop propagation for touch events inside popup (mobile)
