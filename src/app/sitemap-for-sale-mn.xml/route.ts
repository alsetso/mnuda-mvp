/**
 * Dynamic sitemap for Minnesota for-sale localities
 * /sitemap-for-sale-mn.xml
 */
...
export async function GET() {
  ...
  if (cities) {
    for (const city of cities) {
      ...
      urls.push(`  <url>
    <loc>${baseUrl}/mn/${city.slug}?status=for-sale</loc>
  ...
  if (counties) {
    for (const county of counties) {
      ...
      urls.push(`  <url>
    <loc>${baseUrl}/mn/county/${county.slug}?status=for-sale</loc>
    <lastmod>${lastmod}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.7</priority>
  </url>`);
  ...
  if (zips) {
    for (const zip of zips) {
      ...
      urls.push(`  <url>
    <loc>${baseUrl}/mn/zip/${zip.zip_code}?status=for-sale</loc>
    <lastmod>${lastmod}</lastmod>
    <changefreq>weekly</changefreq>
