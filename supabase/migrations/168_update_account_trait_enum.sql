-- Update account_trait enum to new values
-- Removes: resident, broker, contractor, services, business, landlord, manager, developer
-- Adds: title
-- Keeps: homeowner, buyer, investor, realtor, wholesaler, lender
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Only keep valid traits that exist in new enum
      IF trait_text IN ('homeowner', 'buyer', 'investor', 'realtor', 'wholesaler', 'lender') THEN
        BEGIN
...
COMMENT ON COLUMN public.accounts.traits IS
  'Array of account traits that help define the account''s activities on MNUDA. Users can select multiple traits from: homeowner, buyer, investor, realtor, wholesaler, lender, title.';
