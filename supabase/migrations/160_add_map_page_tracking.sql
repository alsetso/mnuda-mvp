-- Add map page tracking support
-- Enables tracking for /map page with same three metrics as feed and business pages

-- ============================================================================
-- STEP 1: Update record_page_view to support map page
-- ============================================================================
...
BEGIN
  ...
  IF p_entity_type NOT IN ('post', 'article', 'city', 'county', 'account', 'business', 'feed', 'map') THEN
  ...
  ELSIF p_entity_slug IS NOT NULL THEN
    ...
    ELSIF p_entity_type IN ('city', 'county') THEN
      ...
      USING p_entity_slug
      INTO v_entity_id_for_update;
    ELSIF p_entity_type IN ('business', 'feed', 'map') THEN
      -- Business/Feed/Map pages: 'business', 'directory', 'feed', 'map' slugs don't resolve to entity_id
    ...
    END IF;
  ELSE
    -- For feed, business, and map page slugs, allow NULL entity_id
    IF p_entity_type IN ('feed', 'business', 'map') THEN
      v_entity_id_for_update := NULL;
    ELSE
...
-- ============================================================================
-- STEP 2: Create function to get map page statistics
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_map_page_stats(
...
BEGIN
  RETURN QUERY
  SELECT 
    -- Total loads: count of all page views for the map page within time period
  ...
  WHERE entity_type = 'map'
    AND viewed_at >= NOW() - (p_hours || ' hours')::INTERVAL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Grant permissions
GRANT EXECUTE ON FUNCTION public.get_map_page_stats TO anon, authenticated;

-- Add comment
COMMENT ON FUNCTION public.get_map_page_stats IS
  'Returns map page statistics: total_loads (all views), unique_visitors (distinct accounts + IPs), and accounts_active (distinct accounts only). p_hours parameter filters to last N hours (default 24).';

-- ============================================================================
-- STEP 3: Allow anonymous users to read map page stats
-- ============================================================================

-- Drop policy if it exists (for idempotency)
DROP POLICY IF EXISTS "anon_can_read_map_page_stats" ON public.page_views;

-- Add policy for anonymous users to read map page views (for stats only)
CREATE POLICY "anon_can_read_map_page_stats"
  ON public.page_views FOR SELECT
  TO anon
  USING (entity_type = 'map');
